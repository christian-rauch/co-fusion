...
//uniform float unaryConfWeighting;
...
layout(location = 1) out vec4 vertexConf;
layout(location = 2) out vec4 normalRadius;
layout(location = 3) out uint time;
//layout(location = 4) out float weightedConf; // ~~Used for unrary terms during segmentation~~ Now using vertexConf
...
void main()
{
    ...
    //weightedConf = position.w * unaryConfWeighting;

    normalRadius = normRad;
